...
**Choose one:**
- **Option A: IPMI** (Step 10.1-10.5) - Uses IPMI protocol with ports 6230-6232
- **Option B: Redfish** (Step 10.6-10.10) - Uses Redfish protocol with ports 8000-8002

---

## Option A: Using IPMI

#### 10.1 Get Required Information (IPMI)
...
#### 10.3 Create BareMetalHost for node-0 (IPMI)
...
#### 10.4 Create BareMetalHost for node-1 (IPMI)
...
#### 10.5 Create BareMetalHost for node-2 (IPMI)
...
## Option B: Using Redfish

#### 10.6 Get Required Information (Redfish)

From the OpenStack VM, collect:
- **VM External IP**: `${VM_IP}` (your OpenStack VM floating IP)
- **MAC Addresses**: From Step 7.5
- **System IDs**: libvirt VM UUIDs (see Step 10.7)

#### 10.7 Get Redfish System IDs

The system ID is the **libvirt VM UUID**. Get it from the OpenStack VM:

```bash
# On OpenStack VM, get system IDs (UUIDs) for each VM
NODE_0_SYSTEM_ID=$(sudo virsh dominfo node-0 | grep UUID | awk '{print $2}')
NODE_1_SYSTEM_ID=$(sudo virsh dominfo node-1 | grep UUID | awk '{print $2}')
NODE_2_SYSTEM_ID=$(sudo virsh dominfo node-2 | grep UUID | awk '{print $2}')

echo "Node-0 System ID: $NODE_0_SYSTEM_ID"
echo "Node-1 System ID: $NODE_1_SYSTEM_ID"
echo "Node-2 System ID: $NODE_2_SYSTEM_ID"

# Verify Redfish endpoints are accessible
curl http://localhost:8000/redfish/v1/Systems/$NODE_0_SYSTEM_ID
curl http://localhost:8000/redfish/v1/Systems/$NODE_1_SYSTEM_ID
curl http://localhost:8000/redfish/v1/Systems/$NODE_2_SYSTEM_ID
```

**Note**: The system ID is the UUID from `virsh dominfo`. See `docs/GET-REDFISH-SYSTEM-ID.md` for more details.

#### 10.8 Create BMC Credentials Secret (Redfish)
...
#### 10.9 Create BareMetalHost for node-0 (Redfish)

```bash
# From your Rancher cluster
export VM_IP="192.168.1.100"  # Your OpenStack VM IP
export NODE_0_MAC="52:54:00:xx:xx:xx"  # Replace with actual MAC from Step 7.5
export NODE_0_SYSTEM_ID="492fcbab-4a79-40d7-8fea-a7835a05ef4a"  # Replace with actual UUID from Step 10.7
...
#### 10.10 Create BareMetalHost for node-1 (Redfish)

```bash
# From your Rancher cluster
export VM_IP="192.168.1.100"  # Your OpenStack VM IP
export NODE_1_MAC="52:54:00:yy:yy:yy"  # Replace with actual MAC from Step 7.5
export NODE_1_SYSTEM_ID="882533c5-2f14-49f6-aa44-517e1e404fd8"  # Replace with actual UUID from Step 10.7
...
spec:
  ...
  bootMACAddress: ${NODE_1_MAC}
  bmc:
    address: redfish+http://${VM_IP}:8000/redfish/v1/Systems/${NODE_1_SYSTEM_ID}
    credentialsName: bmc-credentials
    disableCertificateVerification: true
  bootMode: legacy
  automatedCleaningMode: metadata
...
#### 10.11 Create BareMetalHost for node-2 (Redfish)

```bash
# From your Rancher cluster
export VM_IP="192.168.1.100"  # Your OpenStack VM IP
export NODE_2_MAC="52:54:00:zz:zz:zz"  # Replace with actual MAC from Step 7.5
export NODE_2_SYSTEM_ID="a1b2c3d4-e5f6-7890-abcd-ef1234567890"  # Replace with actual UUID from Step 10.7
...
spec:
  ...
  bootMACAddress: ${NODE_2_MAC}
  bmc:
    address: redfish+http://${VM_IP}:8000/redfish/v1/Systems/${NODE_2_SYSTEM_ID}
    credentialsName: bmc-credentials
    disableCertificateVerification: true
  bootMode: legacy
  automatedCleaningMode: metadata
...
**Key Differences: IPMI vs Redfish**

| Aspect | IPMI | Redfish |
|--------|------|---------|
| **BMC Address Format** | `ipmi://host:port` | `redfish+http://host:port/redfish/v1/Systems/{system-id}` |
| **Ports** | 6230, 6231, 6232 | 8000 (single port for all VMs) |
| **System ID Required** | ❌ No | ✅ Yes (libvirt VM UUID) |
| **Protocol** | IPMI 2.0 | REST API (Redfish) |
| **ISO Support** | Limited | ✅ Full VirtualMedia support |

**See `docs/REDFISH-VS-IPMI-COMPARISON.md` for detailed comparison.**
